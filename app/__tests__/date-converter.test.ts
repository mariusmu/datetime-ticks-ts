import { convertToCsharpTick, convertToString } from "../helpers/date-converter";
import { convertToJSTickFromCsharpTick } from "../helpers/date-converter";

test("convert to c# tick from js date 2020-04-28 00:00 +2GMT", () => {
    expect(convertToCsharpTick(1588032000000)).toBe("637236288000000000")
});

test("convert to c# tick from js date 2020-12-15 00:00 +1GMT", () => {
    expect(convertToCsharpTick(1607990400000)).toBe("637435872000000000")
});

test("convert to js tick from c# date 2020-04-28 00:00 +2GMT", () => {
    expect(convertToJSTickFromCsharpTick("637236288000000000")).toBe(1588032000000);
});

test("convert to js tick from c# date 2020-12-15 00:00 +1GMT", () => {
    expect(convertToJSTickFromCsharpTick("637435872000000000")).toBe(1607990400000);
});

test("convert to javascript human readable date string 2020-04-28 ", () => {
